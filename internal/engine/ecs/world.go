...
type World struct {
	...
	Systems          []System
	Names            map[EntityID]scene.NameComponent
	Transforms       map[EntityID]scene.TransformComponent
	Bodies           map[EntityID]physics.BodyComponent
	PrimitiveRenders map[EntityID]scene.PrimitiveRenderComponent
}
...
func NewWorld(config WorldConfig) *World {
	return &World{
		...
		Names:            make(map[EntityID]scene.NameComponent),
		Transforms:       make(map[EntityID]scene.TransformComponent),
		Bodies:           make(map[EntityID]physics.BodyComponent),
		PrimitiveRenders: make(map[EntityID]scene.PrimitiveRenderComponent),
	}
}
