package scenario

import (
	"encoding/binary"
	"hash/fnv"

	"server2/internal/fixed"
	"server2/internal/geometry"
	"server2/internal/physics"
)

func HashSceneState(state SceneState) uint64 {
	hasher := fnv.New64a()
	buffer := make([]byte, 8)

	writeUint64 := func(value uint64) {
		binary.LittleEndian.PutUint64(buffer, value)
		_, _ = hasher.Write(buffer)
	}

	writeInt64 := func(value int64) {
		binary.LittleEndian.PutUint64(buffer, uint64(value))
		_, _ = hasher.Write(buffer)
	}

	writeBool := func(value bool) {
		if value {
			writeUint64(1)
			return
		}
		writeUint64(0)
	}

	writeFixed := func(value fixed.Fixed) {
		writeInt64(value.Raw())
	}

	writeVector3 := func(value geometry.Vector3) {
		writeFixed(value.X)
		writeFixed(value.Y)
		writeFixed(value.Z)
	}

	writeTriangle := func(value geometry.Triangle) {
		writeVector3(value.A)
		writeVector3(value.B)
		writeVector3(value.C)
	}

	writeContact := func(value physics.SphereTriangleContact) {
		writeBool(value.Hit)
		writeVector3(value.Point)
		writeVector3(value.Normal)
		writeFixed(value.Penetration)
	}

	writeUint64(state.Tick)
	writeVector3(state.Sphere.Motion.Position)
	writeVector3(state.Sphere.Motion.Velocity)
	writeVector3(state.Sphere.Motion.AccumulatedForce)
	writeFixed(state.Sphere.Motion.Mass)
	writeFixed(state.Sphere.Motion.InverseMass)
	writeFixed(state.Sphere.Radius)
	writeFixed(state.Sphere.Restitution)
	writeFixed(state.Sphere.Friction)
	writeBool(state.Sphere.Grounded)
	writeBool(state.EverTouchedGround)
	writeBool(state.BounceDetected)
	writeBool(state.SphereSphereCollisionDetected)
	writeBool(state.SphereBoxCollisionDetected)
	writeBool(state.BoxBoxCollisionDetected)
	writeUint64(uint64(state.BroadphaseCellCount))
	writeUint64(uint64(state.SphereSphereCandidateCount))
	writeUint64(uint64(state.BoxBoxCandidateCount))
	writeUint64(uint64(state.SphereBoxCandidateCount))
	writeUint64(uint64(state.SphereSphereHitCount))
	writeUint64(uint64(state.BoxBoxHitCount))
	writeUint64(uint64(state.SphereBoxHitCount))
	writeBool(state.CCDContactDetected)
	writeFixed(state.CCDTimeOfImpact)
	writeUint64(uint64(state.SleepingSphereCount))
	writeUint64(uint64(state.SleepingBoxCount))
	writeUint64(uint64(state.ActiveCCDSphereCount))
	writeUint64(uint64(state.ActiveCCDBoxCount))
	writeUint64(uint64(state.ActiveAngularRiskCCDBoxCount))
	writeUint64(uint64(state.ActiveDiscreteSphereCount))
	writeUint64(uint64(state.ActiveDiscreteBoxCount))
	writeBool(state.EverActivatedCCDSphere)
	writeBool(state.EverActivatedCCDBox)
	writeBool(state.EverActivatedAngularRiskCCDBox)
	writeBool(state.EverSleptSphere)
	writeBool(state.EverSleptBox)
	writeUint64(uint64(state.SleepingSphereSphereSkipCount))
	writeUint64(uint64(state.SleepingBoxBoxSkipCount))
	writeUint64(uint64(state.SleepingSphereBoxSkipCount))
	writeBool(state.EverSkippedSleepingPairs)
	writeUint64(uint64(state.BoxCCDPrecheckCount))
	writeUint64(uint64(state.BoxCCDPrecheckRejectCount))
	writeUint64(uint64(state.BoxCCDMeshSweepCount))
	writeBool(state.EverRanBoxCCDPrecheck)
	writeBool(state.EverRejectedBoxCCDPrecheck)
	writeBool(state.EverExecutedBoxCCDMeshSweep)
	writeUint64(uint64(state.SphereCCDPrecheckCount))
	writeUint64(uint64(state.SphereCCDPrecheckRejectCount))
	writeUint64(uint64(state.SphereCCDMeshSweepCount))
	writeBool(state.EverRanSphereCCDPrecheck)
	writeBool(state.EverRejectedSphereCCDPrecheck)
	writeBool(state.EverExecutedSphereCCDMeshSweep)
	writeUint64(uint64(state.SphereCCDHysteresisHoldCount))
	writeUint64(uint64(state.BoxCCDHysteresisHoldCount))
	writeUint64(uint64(state.SphereSleepHysteresisHoldCount))
	writeUint64(uint64(state.BoxSleepHysteresisHoldCount))
	writeBool(state.EverHeldSphereCCDHysteresis)
	writeBool(state.EverHeldBoxCCDHysteresis)
	writeBool(state.EverHeldSphereSleepHysteresis)
	writeBool(state.EverHeldBoxSleepHysteresis)
	writeUint64(uint64(state.PersistentPairReuseCount))
	writeUint64(uint64(state.WarmStartedPairCount))
	writeUint64(uint64(state.SleepingIslandCount))
	writeUint64(uint64(state.LargestSleepingIslandSize))
	writeBool(state.EverReusedPersistentPairs)
	writeBool(state.EverWarmStartedPairs)
	writeBool(state.EverBuiltSleepingIsland)
	writeBool(state.EverSleptIsland)
	writeUint64(uint64(state.SolverPass1PairCount))
	writeUint64(uint64(state.SolverPass2PairCount))
	writeBool(state.EverReducedSolverPass2)
	writeUint64(uint64(state.CCDBudgetSkipCount))
	writeUint64(uint64(state.SolverFrontierBudgetCount))
	writeBool(state.EverAppliedCCDBudget)
	writeBool(state.EverAppliedSolverBudget)
	writeVector3(state.VehicleChassis.Motion.Position)
	writeVector3(state.VehicleChassis.Motion.Velocity)
	writeVector3(state.VehicleChassis.Motion.AccumulatedForce)
	writeFixed(state.VehicleChassis.Motion.Mass)
	writeFixed(state.VehicleChassis.Motion.InverseMass)
	writeVector3(state.VehicleChassis.HalfExtents)
	writeFixed(state.VehicleChassis.Restitution)
	writeBool(state.VehicleChassis.Grounded)
	writeFixed(state.VehicleChassis.Orientation.W)
	writeFixed(state.VehicleChassis.Orientation.X)
	writeFixed(state.VehicleChassis.Orientation.Y)
	writeFixed(state.VehicleChassis.Orientation.Z)
	writeVector3(state.VehicleChassis.AngularVelocity)
	writeVector3(state.VehicleChassis.InverseInertiaBody)
	writeUint64(uint64(len(state.VehicleProbeLocalOffsets)))
	for _, offset := range state.VehicleProbeLocalOffsets {
		writeVector3(offset)
	}
	writeFixed(state.VehicleWheelRadius)
	writeFixed(state.VehicleWheelWidth)
	writeBool(state.VehicleUseWheelCollider)
	writeFixed(state.VehicleWheelCorrectionClamp)
	writeUint64(uint64(state.VehicleGroundedProbeCount))
	writeFixed(state.VehicleAverageCompression)
	writeFixed(state.VehicleUprightDot)
	writeBool(state.VehicleSettled)
	writeBool(state.VehicleEverHitWall)
	writeFixed(state.VehicleThrottleInput)
	writeFixed(state.VehicleSteerInput)
	writeUint64(uint64(state.VehicleFrontGroundedProbeCount))
	writeUint64(uint64(state.VehicleRearGroundedProbeCount))
	writeFixed(state.VehicleSuspensionRestLength)
	writeFixed(state.VehicleSuspensionSweepDistance)
	writeFixed(state.VehicleSuspensionSpring)
	writeFixed(state.VehicleSuspensionDamper)
	writeFixed(state.VehicleDriveForce)
	writeFixed(state.VehicleEngineBrake)
	writeFixed(state.VehicleFrontDriveShare)
	writeFixed(state.VehicleFrontGrip)
	writeFixed(state.VehicleRearGrip)
	writeFixed(state.VehicleAntiRoll)
	writeFixed(state.VehicleMaxSteerAngle)
	writeFixed(state.VehicleRearSteerAssist)
	writeUint64(uint64(len(state.VehicleWheelGroundedGraceTicks)))
	for _, ticks := range state.VehicleWheelGroundedGraceTicks {
		writeUint64(uint64(ticks))
	}
	writeBool(state.VehicleWallContactActive)
	writeUint64(uint64(len(state.VehicleWheelProbes)))
	for _, probe := range state.VehicleWheelProbes {
		writeVector3(probe.LocalOffset)
		writeVector3(probe.WorldPosition)
		writeVector3(probe.ContactPoint)
		writeVector3(probe.ContactNormal)
		writeVector3(probe.WheelRight)
		writeFixed(probe.Compression)
		writeBool(probe.Grounded)
	}
	writeUint64(uint64(len(state.PersistentContactCache)))
	for _, entry := range state.PersistentContactCache {
		writeUint64(entry.Key)
		writeVector3(entry.Point)
		writeVector3(entry.Normal)
		writeFixed(entry.NormalImpulse)
		writeVector3(entry.TangentImpulse)
	}
	writeUint64(uint64(len(state.BounceDetectedSet)))
	for _, value := range state.BounceDetectedSet {
		writeBool(value)
	}
	writeFixed(state.PeakBounceHeight)
	writeUint64(uint64(len(state.PeakBounceHeights)))
	for _, value := range state.PeakBounceHeights {
		writeFixed(value)
	}
	writeContact(state.LastContact)
	writeUint64(uint64(len(state.Spheres)))
	for _, sphere := range state.Spheres {
		writeVector3(sphere.Motion.Position)
		writeVector3(sphere.Motion.Velocity)
		writeVector3(sphere.Motion.AccumulatedForce)
		writeFixed(sphere.Motion.Mass)
		writeFixed(sphere.Motion.InverseMass)
		writeFixed(sphere.Radius)
		writeFixed(sphere.Restitution)
		writeFixed(sphere.Friction)
		writeBool(sphere.Grounded)
	}
	writeVector3(state.RigidSphere.Motion.Position)
	writeVector3(state.RigidSphere.Motion.Velocity)
	writeVector3(state.RigidSphere.Motion.AccumulatedForce)
	writeFixed(state.RigidSphere.Motion.Mass)
	writeFixed(state.RigidSphere.Motion.InverseMass)
	writeFixed(state.RigidSphere.Radius)
	writeFixed(state.RigidSphere.Restitution)
	writeFixed(state.RigidSphere.Friction)
	writeBool(state.RigidSphere.UseCCD)
	writeUint64(uint64(state.RigidSphere.CCDMode))
	writeBool(state.RigidSphere.CCDActive)
	writeFixed(state.RigidSphere.CCDVelocityThreshold)
	writeFixed(state.RigidSphere.SleepLinearThreshold)
	writeFixed(state.RigidSphere.SleepAngularThreshold)
	writeUint64(uint64(state.RigidSphere.SleepTickThreshold))
	writeUint64(uint64(state.RigidSphere.SleepTickCount))
	writeBool(state.RigidSphere.Sleeping)
	writeBool(state.RigidSphere.Grounded)
	writeFixed(state.RigidSphere.Orientation.W)
	writeFixed(state.RigidSphere.Orientation.X)
	writeFixed(state.RigidSphere.Orientation.Y)
	writeFixed(state.RigidSphere.Orientation.Z)
	writeVector3(state.RigidSphere.AngularVelocity)
	writeFixed(state.RigidSphere.InverseInertia)
	writeBool(state.RigidSphereBounceDetected)
	writeBool(state.RigidSphereCCDHitDetected)
	writeBool(state.RigidSphereSphereCollisionDetected)
	writeFixed(state.RigidSpherePeakBounceHeight)
	writeBool(state.RigidSphereRotationChanged)
	writeUint64(uint64(len(state.RigidSphereTouchedGroundSet)))
	for _, value := range state.RigidSphereTouchedGroundSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.RigidSpheres)))
	for _, sphere := range state.RigidSpheres {
		writeVector3(sphere.Motion.Position)
		writeVector3(sphere.Motion.Velocity)
		writeVector3(sphere.Motion.AccumulatedForce)
		writeFixed(sphere.Motion.Mass)
		writeFixed(sphere.Motion.InverseMass)
		writeFixed(sphere.Radius)
		writeFixed(sphere.Restitution)
		writeFixed(sphere.Friction)
		writeBool(sphere.UseCCD)
		writeUint64(uint64(sphere.CCDMode))
		writeBool(sphere.CCDActive)
		writeFixed(sphere.CCDVelocityThreshold)
		writeFixed(sphere.SleepLinearThreshold)
		writeFixed(sphere.SleepAngularThreshold)
		writeUint64(uint64(sphere.SleepTickThreshold))
		writeUint64(uint64(sphere.SleepTickCount))
		writeBool(sphere.Sleeping)
		writeBool(sphere.Grounded)
		writeFixed(sphere.Orientation.W)
		writeFixed(sphere.Orientation.X)
		writeFixed(sphere.Orientation.Y)
		writeFixed(sphere.Orientation.Z)
		writeVector3(sphere.AngularVelocity)
		writeFixed(sphere.InverseInertia)
	}
	writeUint64(uint64(len(state.RigidSphereBounceDetectedSet)))
	for _, value := range state.RigidSphereBounceDetectedSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.RigidSpherePeakBounceHeights)))
	for _, value := range state.RigidSpherePeakBounceHeights {
		writeFixed(value)
	}
	writeUint64(uint64(len(state.RigidSphereRotationChangedSet)))
	for _, value := range state.RigidSphereRotationChangedSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.LastContacts)))
	for _, contact := range state.LastContacts {
		writeContact(contact)
	}
	writeVector3(state.Box.Motion.Position)
	writeVector3(state.Box.Motion.Velocity)
	writeVector3(state.Box.Motion.AccumulatedForce)
	writeFixed(state.Box.Motion.Mass)
	writeFixed(state.Box.Motion.InverseMass)
	writeVector3(state.Box.HalfExtents)
	writeFixed(state.Box.Restitution)
	writeFixed(state.Box.Friction)
	writeFixed(state.Box.RotationZ)
	writeFixed(state.Box.AngularVelocityZ)
	writeFixed(state.Box.InverseInertiaZ)
	writeBool(state.Box.Grounded)
	writeUint64(uint64(len(state.Boxes)))
	for _, box := range state.Boxes {
		writeVector3(box.Motion.Position)
		writeVector3(box.Motion.Velocity)
		writeVector3(box.Motion.AccumulatedForce)
		writeFixed(box.Motion.Mass)
		writeFixed(box.Motion.InverseMass)
		writeVector3(box.HalfExtents)
		writeFixed(box.Restitution)
		writeFixed(box.Friction)
		writeFixed(box.RotationZ)
		writeFixed(box.AngularVelocityZ)
		writeFixed(box.InverseInertiaZ)
		writeBool(box.Grounded)
	}
	writeUint64(uint64(len(state.GroundBoxes)))
	for _, box := range state.GroundBoxes {
		writeVector3(box.Min)
		writeVector3(box.Max)
	}
	writeUint64(uint64(len(state.BoxBounceDetectedSet)))
	for _, value := range state.BoxBounceDetectedSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.BoxPeakBounceHeights)))
	for _, value := range state.BoxPeakBounceHeights {
		writeFixed(value)
	}
	writeUint64(uint64(len(state.BoxInitialRotations)))
	for _, value := range state.BoxInitialRotations {
		writeFixed(value)
	}
	writeUint64(uint64(len(state.BoxRotationChangedSet)))
	for _, value := range state.BoxRotationChangedSet {
		writeBool(value)
	}
	writeVector3(state.RigidBox.Motion.Position)
	writeVector3(state.RigidBox.Motion.Velocity)
	writeVector3(state.RigidBox.Motion.AccumulatedForce)
	writeFixed(state.RigidBox.Motion.Mass)
	writeFixed(state.RigidBox.Motion.InverseMass)
	writeVector3(state.RigidBox.HalfExtents)
	writeFixed(state.RigidBox.Restitution)
	writeBool(state.RigidBox.UseCCD)
	writeUint64(uint64(state.RigidBox.CCDMode))
	writeBool(state.RigidBox.CCDActive)
	writeFixed(state.RigidBox.CCDVelocityThreshold)
	writeFixed(state.RigidBox.CCDAngularSweepThreshold)
	writeFixed(state.RigidBox.SleepLinearThreshold)
	writeFixed(state.RigidBox.SleepAngularThreshold)
	writeUint64(uint64(state.RigidBox.SleepTickThreshold))
	writeUint64(uint64(state.RigidBox.SleepTickCount))
	writeBool(state.RigidBox.Sleeping)
	writeBool(state.RigidBox.Grounded)
	writeFixed(state.RigidBox.Orientation.W)
	writeFixed(state.RigidBox.Orientation.X)
	writeFixed(state.RigidBox.Orientation.Y)
	writeFixed(state.RigidBox.Orientation.Z)
	writeVector3(state.RigidBox.AngularVelocity)
	writeVector3(state.RigidBox.InverseInertiaBody)
	writeBool(state.RigidBoxBounceDetected)
	writeBool(state.RigidBoxCCDHitDetected)
	writeBool(state.RigidBoxBoxCollisionDetected)
	writeFixed(state.RigidBoxPeakBounceHeight)
	writeBool(state.RigidBoxRotationChanged)
	writeUint64(uint64(len(state.RigidBoxes)))
	for _, box := range state.RigidBoxes {
		writeVector3(box.Motion.Position)
		writeVector3(box.Motion.Velocity)
		writeVector3(box.Motion.AccumulatedForce)
		writeFixed(box.Motion.Mass)
		writeFixed(box.Motion.InverseMass)
		writeVector3(box.HalfExtents)
		writeFixed(box.Restitution)
		writeBool(box.UseCCD)
		writeUint64(uint64(box.CCDMode))
		writeBool(box.CCDActive)
		writeFixed(box.CCDVelocityThreshold)
		writeFixed(box.CCDAngularSweepThreshold)
		writeFixed(box.SleepLinearThreshold)
		writeFixed(box.SleepAngularThreshold)
		writeUint64(uint64(box.SleepTickThreshold))
		writeUint64(uint64(box.SleepTickCount))
		writeBool(box.Sleeping)
		writeBool(box.Grounded)
		writeFixed(box.Orientation.W)
		writeFixed(box.Orientation.X)
		writeFixed(box.Orientation.Y)
		writeFixed(box.Orientation.Z)
		writeVector3(box.AngularVelocity)
		writeVector3(box.InverseInertiaBody)
	}
	writeUint64(uint64(len(state.RigidBoxBounceDetectedSet)))
	for _, value := range state.RigidBoxBounceDetectedSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.RigidBoxPeakBounceHeights)))
	for _, value := range state.RigidBoxPeakBounceHeights {
		writeFixed(value)
	}
	writeUint64(uint64(len(state.RigidBoxRotationChangedSet)))
	for _, value := range state.RigidBoxRotationChangedSet {
		writeBool(value)
	}
	writeUint64(uint64(len(state.GroundTriangles)))
	for _, triangle := range state.GroundTriangles {
		writeTriangle(triangle)
	}

	return hasher.Sum64()
}
