...
func TestSweepSphereAxisAlignedBoundingBoxDetectsThinWallHit(t *testing.T) {
	...
	if !contact.Hit {
		t.Fatalf("expected swept sphere to hit the thin wall")
	}
	if contact.TimeOfImpact.Cmp(fixed.Zero) <= 0 || contact.TimeOfImpact.Cmp(fixed.One) >= 0 {
		t.Fatalf("expected time of impact to be inside the step, got %v", contact.TimeOfImpact)
	}
	if contact.Normal.X.Cmp(fixed.One.Neg()) != 0 {
		t.Fatalf("expected hit normal to face back along X, got %v", contact.Normal)
	}
}
