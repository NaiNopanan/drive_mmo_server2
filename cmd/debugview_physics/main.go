...
func drawSceneObjectLabels(camera rl.Camera3D, state scenario.SceneState) {
	...
	for index, sphere := range sceneRigidSpheres(state) {
		lines := []string{
			...
			fmt.Sprintf("mu=%s", fixedToText(sphere.Friction)),
			fmt.Sprintf("vx=%s vy=%s", fixedToText(sphere.Motion.Velocity.X), fixedToText(sphere.Motion.Velocity.Y)),
			fmt.Sprintf("ccd=%s", ccdLabel(sphere.UseCCD, sphere.CCDMode)),
	...
	for index, box := range sceneRigidBoxes(state) {
		lines := []string{
			...
			fmt.Sprintf("m=%s", fixedToText(box.Motion.Mass)),
			fmt.Sprintf("vx=%s vy=%s", fixedToText(box.Motion.Velocity.X), fixedToText(box.Motion.Velocity.Y)),
			fmt.Sprintf("ccd=%s", ccdLabel(box.UseCCD, box.CCDMode)),
...
func shouldDrawObjectLabels(definition scenario.ScenarioDefinition) bool {
	return definition.Name != "Hundred Rigid Spheres In Box" &&
		...
		definition.Name != "Hundred Rigid Spheres And Hundred Rigid Boxes In Box All CCD" &&
...
func drawOverlayWithCameraMode(definition scenario.ScenarioDefinition, runner *scenario.ScenarioRunner, viewMode cameraViewMode, sceneIndex int, sceneCount int) {
	state := runner.State
	...
	if state.ActiveAngularRiskCCDBoxCount > 0 || state.SleepingSphereSphereSkipCount > 0 || state.SleepingBoxBoxSkipCount > 0 || state.SleepingSphereBoxSkipCount > 0 {
		overlayHeight += 56
	}
	if state.BoxCCDPrecheckCount > 0 || state.BoxCCDPrecheckRejectCount > 0 || state.BoxCCDMeshSweepCount > 0 {
		overlayHeight += 56
	}
	if state.SphereCCDPrecheckCount > 0 || state.SphereCCDPrecheckRejectCount > 0 || state.SphereCCDMeshSweepCount > 0 {
		overlayHeight += 56
	}
	if state.SphereCCDHysteresisHoldCount > 0 || state.BoxCCDHysteresisHoldCount > 0 || state.SphereSleepHysteresisHoldCount > 0 || state.BoxSleepHysteresisHoldCount > 0 {
		overlayHeight += 56
	}
	...
	rl.DrawText(fmt.Sprintf("Scene %d / %d", sceneIndex+1, sceneCount), 30, 30, 20, rl.DarkGray)
	rl.DrawText(definition.Name, 30, 54, 28, rl.DarkBlue)
	rl.DrawText(definition.Description, 30, 90, 18, rl.Black)

	status := statusLabel(runner.LastResult.Status)
	color := statusColor(runner.LastResult.Status)
	rl.DrawRectangle(30, 120, 110, 28, color)
	...
	if state.SleepingSphereCount > 0 || state.SleepingBoxCount > 0 || state.ActiveCCDSphereCount > 0 || state.ActiveCCDBoxCount > 0 || state.ActiveDiscreteSphereCount > 0 || state.ActiveDiscreteBoxCount > 0 {
		rl.DrawText(
			fmt.Sprintf("Sleeping S/B: %d / %d", state.SleepingSphereCount, state.SleepingBoxCount),
			30,
			infoY,
			18,
			rl.Black,
		...
		rl.DrawText(
			fmt.Sprintf("Active CCD S/B: %d / %d | Discrete S/B: %d / %d", state.ActiveCCDSphereCount, state.ActiveCCDBoxCount, state.ActiveDiscreteSphereCount, state.ActiveDiscreteBoxCount),
			30,
			infoY+28,
			18,
			rl.Black,
	...
	if state.ActiveAngularRiskCCDBoxCount > 0 || state.SleepingSphereSphereSkipCount > 0 || state.SleepingBoxBoxSkipCount > 0 || state.SleepingSphereBoxSkipCount > 0 {
		rl.DrawText(
			fmt.Sprintf("Angular-risk CCD boxes: %d", state.ActiveAngularRiskCCDBoxCount),
			30,
			infoY,
			18,
			rl.Black,
		...
		rl.DrawText(
			fmt.Sprintf("Sleep skips SS/BB/SB: %d / %d / %d", state.SleepingSphereSphereSkipCount, state.SleepingBoxBoxSkipCount, state.SleepingSphereBoxSkipCount),
			30,
			infoY+28,
			18,
			rl.Black,
	...
	if state.SphereCCDPrecheckCount > 0 || state.SphereCCDPrecheckRejectCount > 0 || state.SphereCCDMeshSweepCount > 0 {
		rl.DrawText(
			fmt.Sprintf("Sphere CCD precheck: %d | reject: %d", state.SphereCCDPrecheckCount, state.SphereCCDPrecheckRejectCount),
			30,
			infoY,
			18,
			rl.Black,
		...
		rl.DrawText(
			fmt.Sprintf("Sphere full mesh sweeps: %d", state.SphereCCDMeshSweepCount),
			30,
			infoY+28,
			18,
			rl.Black,
	...
	if state.SphereCCDHysteresisHoldCount > 0 || state.BoxCCDHysteresisHoldCount > 0 || state.SphereSleepHysteresisHoldCount > 0 || state.BoxSleepHysteresisHoldCount > 0 {
		rl.DrawText(
			fmt.Sprintf("CCD hysteresis hold S/B: %d / %d", state.SphereCCDHysteresisHoldCount, state.BoxCCDHysteresisHoldCount),
			30,
			infoY,
			18,
			rl.Black,
		...
		rl.DrawText(
			fmt.Sprintf("Sleep hysteresis hold S/B: %d / %d", state.SphereSleepHysteresisHoldCount, state.BoxSleepHysteresisHoldCount),
			30,
			infoY+28,
			18,
			rl.Black,
...
func main() {
	...
	for !rl.WindowShouldClose() {
		...
		drawOverlayWithCameraMode(definitions[currentIndex], runner, viewMode, currentIndex, len(definitions))
