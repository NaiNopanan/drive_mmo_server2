...
func drawOverlay(definition scenario.ScenarioDefinition, runner *scenario.ScenarioRunner, viewMode cameraViewMode, sceneIndex int, sceneCount int) {
	...
	rl.DrawRectangle(18, 18, 600, 368, rl.Fade(rl.RayWhite, 0.92))
	rl.DrawRectangleLinesEx(rl.NewRectangle(18, 18, 600, 368), 2, rl.DarkGray)
	...
	rl.DrawText(fmt.Sprintf("View: %s", viewLabel(viewMode)), 470, 30, 18, rl.Black)
	...
	rl.DrawText(fmt.Sprintf("Wheel collider: %v", state.VehicleUseWheelCollider), 30, 296, 18, rl.Black)
	rl.DrawText(fmt.Sprintf("Correction clamp: %s", fixedToText(state.VehicleWheelCorrectionClamp)), 30, 322, 18, rl.Black)
	rl.DrawText(fmt.Sprintf("Upright dot: %s | Settled: %v", fixedToText(state.VehicleUprightDot), state.VehicleSettled), 30, 348, 18, rl.Black)
	rl.DrawText(fmt.Sprintf("Hash: %016x", sceneHash), 360, 348, 18, rl.Black)
}

func main() {
	definitions := scenario.Vehicle2ScenarioDefinitions()
	if len(definitions) == 0 {
		panic("debugview_vehicle2: no vehicle scenarios registered")
	}

	rl.InitWindow(1280, 720, "debugview_vehicle2")
